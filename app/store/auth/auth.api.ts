    ...
    return {
      auth: builder.mutation<IAuthResult, IAuthCredentials>({
        ...
        transformResponse: (response: IAuthResult) => {
          Cookies.set(CookiesEnum.authorisation, response.accessToken, {secure: true})
          return response
        }
      }),
      registration: builder.mutation<IAuthResult, IRegCredentials>({
        ...
        transformResponse: (response: IAuthResult) => {
          Cookies.set(CookiesEnum.accessToken, response.accessToken)
          return response
        }
      }),
      validateCode: builder.mutation<IAuthResult, IValidateCode>({
        ...
        transformResponse: (response: IAuthResult) => {
          Cookies.set(CookiesEnum.authorisation, response.accessToken)
          return response
        }
      }),
